...
import {
  renderCoverLetterDocxDocument,
  renderCoverLetterHtml,
} from './templates/cover-letter/index.js';
import { renderPdf } from './core/pdf-renderer.js';
import { renderDocx } from './core/docx-renderer.js';
import { generateCoverLetterBullets } from './core/ollama.js';
import { filenameSlug } from './utils/formatters.js';
import { buildCoverLetterView } from './utils/cover-letter.js';
import { resolvePhoto, toDataUrl, toImageBuffer } from './utils/photo.js';

function parseFormats(format: string): Array<'pdf' | 'docx'> {
  ...
  return formats as Array<'pdf' | 'docx'>;
}
...
async function resolveTemplateId(requested?: string): Promise<string> {
  if (requested?.trim()) {
    getTemplate(requested.trim());
    return requested.trim();
  }
  return promptTemplateId();
}

async function generateCv(opts: {
  input: string;
  template?: string;
  format: string;
  outdir: string;
...
async function generateLetter(opts: {
  company: string;
  role: string;
  jd: string;
  input: string;
  ...
  try {
    jobDescription = await readFile(jdPath, 'utf8');
  } catch {
    throw new Error(`Cannot read job description file: ${jdPath}`);
  }
  if (!jobDescription.trim()) {
    throw new Error(`Job description file is empty: ${jdPath}`);
  }

  console.log(`\nGenerating cover-letter bullets via Ollama (${opts.model})…`);
  ...
  const view = buildCoverLetterView(data, { company, role, bullets });
  const baseName = `${filenameSlug(data.basics.name)}_Cover_Letter_${filenameSlug(company)}`;
  const written: string[] = [];

  if (formats.includes('pdf')) {
    const html = await renderCoverLetterHtml(view);
    const pdfPath = path.join(outdir, `${baseName}.pdf`);
    await renderPdf(html, pdfPath);
    written.push(pdfPath);
  }
  ...
  if (formats.includes('docx')) {
    const doc = await renderCoverLetterDocxDocument(view);
    const docxPath = path.join(outdir, `${baseName}.docx`);
    await renderDocx(doc, docxPath);
    written.push(docxPath);
  }

  console.log(`\nGenerated cover letter for "${role}" at "${company}":`);
  for (const file of written) {
    console.log(`  ${file}`);
  }
}
...
program
  .name('eu-cv')
  .description('Generate A4 PDF and DOCX CVs (and cover letters) from resume JSON for European tech hiring')
  .option('-i, --input <path>', 'Path to resume JSON', './resume.json')
  .option('-t, --template <id>', 'Template id (omit to choose interactively)')
  ...
  .addHelpText(
    ...
    () =>
      `\nTemplates:\n${listTemplates()
        ...
        .join('\n')}\n\nExamples:\n  npm run generate -- -t tech-modern\n  npm run generate -- -t tech-modern --no-photo\n  npm run generate -- -t classic-eu -f pdf\n  npm run generate:letter -- --company Acme --role "Senior Engineer" --jd ./jd.txt\n  npm run generate\n`,
  )
  .action(async (opts) => {
    await generateCv(opts);
  });

program
  .command('letter')
  .description('Generate a tailored cover letter (PDF/DOCX) using local Ollama')
  .requiredOption('--company <name>', 'Target company name')
  .requiredOption('--role <title>', 'Target role title')
  .requiredOption('--jd <path>', 'Path to job description text file')
  .option('-i, --input <path>', 'Path to resume JSON', './resume.json')
  .option('-f, --format <list>', 'Comma-separated formats: pdf,docx', 'pdf,docx')
  .option('-o, --outdir <path>', 'Output directory', './output')
